...
gtc_record_term_visit($term_id, $taxonomy);
$term_meta = get_term_meta($term_id);
$term_info = isset($term_meta['gtc_info'][0]) ? maybe_unserialize($term_meta['gtc_info'][0]) : array();
$all_popular_products = gtc_get_most_popular_products($term_id, $taxonomy);
$personas = gtc_get_term_personas($term_id);
$selected_sub_terms = isset($term_meta['gtc_selected_sub_terms'][0]) ? maybe_unserialize($term_meta['gtc_selected_sub_terms'][0]) : array();
$custom_sub_term_texts = isset($term_meta['gtc_custom_sub_term_texts'][0]) ? maybe_unserialize($term_meta['gtc_custom_sub_term_texts'][0]) : array();
...
$rising_products = gtc_get_rising_products($term_id, $taxonomy, 4);
$custom_description = isset($term_meta['gtc_custom_description'][0]) ? $term_meta['gtc_custom_description'][0] : '';
$custom_header_text = isset($term_meta['gtc_custom_header_text'][0]) ? $term_meta['gtc_custom_header_text'][0] : '';
$gtc_image = isset($term_meta['gtc_image'][0]) ? $term_meta['gtc_image'][0] : '';
$term_name = $term->name;
$term_description = $term->description;
$trending_sub_terms = gtc_get_trending_sub_terms($term_id, $taxonomy, $selected_sub_terms);
function gtc_render_optimized_product_image($product, $index) {
    ...
    printf(
        '<img src="%1$s" alt="%2$s" width="300" height="300" loading="%3$s" decoding="%4$s" fetchpriority="%5$s" class="%6$s" srcset="%7$s" sizes="%8$s">',
        esc_url($image_url), esc_attr($image_alt), $loading, $decoding, $fetchpriority, $class, $srcset, $sizes
    );
}
function gtc_render_optimized_term_image($term_name, $gtc_image) {
    $term_image_url = '';
    if ($gtc_image) {
        $term_image_url = wp_get_attachment_image_url($gtc_image, 'medium');
    }
    $default_image = '/wp-content/uploads/2024/07/category-placeholder.webp';
    printf(
        '<img width="180" height="180" fetchpriority="high" src="%s" alt="%s" class="term-image" loading="eager" decoding="sync">',
        esc_url($term_image_url ?: $default_image), esc_attr($term_name)
    );
}
add_action('wp_head', function() {
    ?>
    <style>
        .product-image-container { aspect-ratio: 1/1; width: 100%; contain: strict; }
        .product-image { width: 100%; height: 100%; object-fit: contain; }
        .term-image { width: 180px; height: 180px; object-fit: cover; }
    </style>
    <?php
}, 1);
?>
<div class="term-card-wrapper" id="term-card" data-term-id="<?php echo esc_attr($term_id); ?>" data-taxonomy="<?php echo esc_attr($taxonomy); ?>">
    <div class="term-card">
        <div class="term-content">
            <div class="term-info-section">
                <div class="term-info-header">
                    <div class="term-image-wrapper">
                      <?php gtc_render_optimized_term_image($term_name, $gtc_image); ?>
                    </div>
                    <div class="term-title-wrapper">
                        <h1><?php echo esc_html($term_name); ?></h1>
                        <?php if (!empty($custom_header_text)): ?>
                            <p class="term-tagline"><?php echo wp_kses_post($custom_header_text); ?></p>
                        <?php else: ?>
                            <p class="term-tagline">Entdecke einzigartige Geschenkideen, die begeistern</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                  ...
                  <div class="product-item">
                    <div class="product-image-container">
                        <a href="<?php echo esc_url(get_permalink($product['id'])); ?>">
                            <?php gtc_render_optimized_product_image($product, $index); ?>
                        </a>
                    </div>
  ...
  <?php if (count($all_popular_products) > $initial_display_count): ?>
  ...
  <?php endif; ?>
              <div class="term-info-content">
                  <div class="info-item">
                      ...
                      <div><?php echo wp_kses_post($term_info['contents'] ?? ''); ?></div>
                  </div>
                  <div class="info-item">
                      <h3><i class="fas fa-users"></i> Für wen?</h3>
                      <div><?php echo wp_kses_post($term_info['target_audience'] ?? ''); ?></div>
                  </div>
                  <div class="info-item">
                      <h3><i class="fas fa-birthday-cake"></i> Altersgruppe</h3>
                      <div><?php echo wp_kses_post($term_info['age_group'] ?? ''); ?></div>
                  </div>
                  <div class="info-item">
                      <h3><i class="fas fa-star"></i> Was ist besonders?</h3>
                      <div><?php echo wp_kses_post($term_info['special_features'] ?? ''); ?></div>
                  </div>
              </div>
              <div class="term-info-footer">
                  <div class="trend-link">
                      ...
                      <?php if (!empty($trending_sub_terms)): ?>
                          <?php foreach ($trending_sub_terms as $sub_term):
                              $sub_term_id = $sub_term['_id'];
                              $sub_term_name = $sub_term['term_name'];
                              $sub_term_url = get_term_link((int) $sub_term_id, $taxonomy);
                              if (is_wp_error($sub_term_url)) {
                                  continue;
                              }
                          ?>
                              <a href="<?php echo esc_url($sub_term_url); ?>" class="trend-button"><?php echo esc_html($sub_term_name); ?></a>
                          <?php endforeach; ?>
                      <?php else: ?>
                          <span class="no-trend">Keine Trend-Begriffe verfügbar.</span>
                      <?php endif; ?>
                  </div>
                  <div class="pinterest-link">
                      ...
                      <?php
                      $pinterest_url = !empty($term_info['pinterest_board_url'])
                          ? $term_info['pinterest_board_url']
        ...
        <div class="random-products-section">
            ...
            <div class="product-grid" id="randomProducts">
                <?php
                $random_products = gtc_get_products($term_id, $taxonomy, 'random', 0, 12);
        ...
        <h2>Alle Geschenkideen - <?php echo $term->count; ?> <?php echo esc_html($term_name); ?></h2>
<script>
jQuery(document).ready(function($) {
        ...
        return `
            <div class="product-item" data-product-id="${product.id}">
                <div class="product-image-container">
                    ...
                    <div class="product-reaction-container">
                        <button class="reaction-button like-button" data-product-id="${product.id}" data-term-id="${termId}">
                            <i class="far fa-heart reaction-icon" aria-hidden="true"></i>
                        </button>
                        <button class="reaction-button dislike-button" data-product-id="${product.id}" data-term-id="${termId}">
                            <i class="far fa-thumbs-down reaction-icon" aria-hidden="true"></i>
                        </button>
                    </div>
                </div>
    ...
    function loadNewProducts() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'gtc_get_random_products',
                term_id: termId,
                taxonomy: taxonomy,
                count: 1,
    ...
    function handleProductReaction(productId, isLike) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'gtc_record_product_feedback',
                nonce: gtc_ajax.nonce,
    ...
    function updatePreviewBox() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'gtc_get_liked_products_info',
...
.awp-add-to-wishlist i {
    color: inherit;
        ...
        <div class="product-grid" id="newProducts">
            <?php
            $new_products = gtc_get_products($term_id, $taxonomy, 'new', 0, 12);
                    ...
                    <div class="product-item">
                        <div class="product-image-container">
                            <a href="<?php echo get_permalink($wc_product['id']); ?>">
                                <?php echo get_the_post_thumbnail($wc_product['id'], 'woocommerce_thumbnail', array('loading'=>'lazy')); ?>
                            </a>
                            <div class="awp-wishlist-button-container">
                                <button class="awp-add-to-wishlist"
                                    data-product-id="<?php echo esc_attr($wc_product['id']); ?>"
                                    data-product-name="<?php echo esc_attr($wc_product['name']); ?>"
                                    data-product-url="<?php echo esc_url(get_permalink($wc_product['id'])); ?>"
                                    aria-label="Zur Wunschliste hinzufügen">
                                    <i class="fa fa-heart" aria-hidden="true"></i>
                                </button>
                            </div>
                        </div>
                        <a class="product-card-link" href="<?php echo get_permalink($wc_product['id']); ?>"><?php echo $wc_product['name']; ?></a>
                        <a rel="nofollow" target="_blank" onclick="_paq.push(['trackEvent', 'Conversion', 'Klick zum Shop', 'Teaser']);"
                           href="/redirect.php?product=<?php echo esc_attr($wc_product['slug']); ?>"
                           class="ansehen-button">
                            <?php
                            $button_text = get_post_meta($wc_product['id'], 'button_text', true);
                            if (empty($button_text)) {
                                $button_text = $wc_product['button_text'];
                            }
        ...
        <div class="product-grid" id="sixMonthsPopularProducts">
            <?php
            $six_months_popular_products = gtc_get_products($term_id, $taxonomy, 'sixMonthsPopular', 0, 4);
        ...
        <h2>Weitere Kategorien passend zu "<?php echo esc_html($term_name); ?>"</h2>
        <div class="sub-terms">
            <?php
            if (!empty($selected_sub_terms)) {
                foreach ($selected_sub_terms as $sub_term_id) {
                    ...
                    if ($sub_term && !is_wp_error($sub_term)) {
                        if (isset($custom_sub_term_texts[$sub_term_id])) {
                            $custom_text = $custom_sub_term_texts[$sub_term_id];
                        } elseif ($sub_term->taxonomy === 'product_cat' && isset($custom_subcategory_texts[$sub_term_id])) {
                            $custom_text = $custom_subcategory_texts[$sub_term_id];
                        } elseif ($sub_term->taxonomy === 'product_tag' && isset($custom_tag_texts[$sub_term_id])) {
                            $custom_text = $custom_tag_texts[$sub_term_id];
                        } else {
                            $custom_text = $sub_term->description;
                        }
                        $custom_text = trim($custom_text); // Entfernt extra Whitespace/leere Zeilen
                        $sub_term_image_id = get_term_meta($sub_term_id, 'gtc_image', true);
                        $image_url = $sub_term_image_id ? wp_get_attachment_image_url($sub_term_image_id, 'full') : '';
                        echo '<a class="term-tag-card" href="' . esc_url(get_term_link($sub_term)) . '">';
                        if ($image_url) {
                            echo '<div class="term-tag-image" style="background-image: url(\'' . esc_url($image_url) . '\');"></div>';
                        } else {
                            echo '<div class="term-tag-image" style="background-image: url(\'' . esc_url('https://geschenkly.de/wp-content/uploads/2024/07/DALL·E-2024-07-26-16.28.35-Create-an-illustration-for-the-Gifts-for-Men-category-designed-to-fit-into-a-placeholder-space-with-a-friendly-and-inviting-vibe.-The-scene-should-.webp') . '\');"></div>';
                        }
                        echo '<div class="term-tag-content">';
                        echo '<h3>' . esc_html($sub_term->name) . '</h3>';
                ...
                echo '<p>Keine Unterbegriffe ausgewählt.</p>';
            }
            ?>
        </div>
        <?php if (!empty($custom_description)): ?>
            <div class="custom-description-card">
                <h3>Wissenswertes zum Thema "<?php echo esc_html($term_name); ?>"</h3>
                <div class="custom-description-content">
                    <?php echo wpautop($custom_description); ?>
                </div>
            </div>
        <?php endif; ?>
        <div class="filter-section term-insights">
<h3>Unsere Partner-Shops für "<?php echo esc_html($term_name); ?>"</h3>
            <div class="filter-buttons partner-grid">
                <?php
                $selected_partner_shops = isset($term_meta['gtc_selected_partner_shops'][0]) ? maybe_unserialize($term_meta['gtc_selected_partner_shops'][0]) : array();
                ...
                if (!empty($selected_partner_shops) && !empty($partner_shops)) {
                    ...
                    echo '<p>Keine Partner-Shops für diesen Begriff ausgewählt.</p>';
                }
                ?>
            </div>
