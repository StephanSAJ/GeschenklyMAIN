...
function wcc_get_mostpopular_products_for_tag_pages_cached($tag_id) {
    $transient_key = 'wcc_mostpopular_products_tag_'.$tag_id;
    $data = get_transient($transient_key);
    if ($data === false) {
        $data = wcc_get_mostpopular_products_for_tag_pages($tag_id);
        set_transient($transient_key, $data, HOUR_IN_SECONDS);
    }
    return $data;
}

function wcc_get_tag_rising_products_cached($tag_id, $limit = 4) {
    $transient_key = 'wcc_rising_products_tag_'.$tag_id.'_'.$limit;
    $data = get_transient($transient_key);
    if ($data === false) {
        $data = wcc_get_tag_rising_products($tag_id, $limit);
        set_transient($transient_key, $data, HOUR_IN_SECONDS);
    }
    return $data;
}

function wcc_get_trending_tags_cached($tag_id, $selected_tags) {
    $transient_key = 'wcc_trending_tags_'.$tag_id;
    $data = get_transient($transient_key);
    if ($data === false) {
        $data = wcc_get_trending_tags($tag_id, $selected_tags);
        set_transient($transient_key, $data, HOUR_IN_SECONDS);
    }
    return $data;
}

function wcc_get_tag_six_months_popular_products_cached($tag_id, $limit = 4) {
    $transient_key = 'wcc_six_months_popular_products_tag_'.$tag_id.'_'.$limit;
    $data = get_transient($transient_key);
    if ($data === false) {
        $data = wcc_get_tag_six_months_popular_products($tag_id, $limit);
        set_transient($transient_key, $data, HOUR_IN_SECONDS);
    }
    return $data;
}

function get_product_count_in_tag_cached($tag_id) {
    $transient_key = 'wcc_product_count_tag_'.$tag_id;
...
function get_random_products_from_tag_cached($tag_id, $count = 12, $exclude = array()) {
    $exclude_key = empty($exclude) ? 'none' : implode('_', $exclude);
    $transient_key = 'wcc_random_products_tag_'.$tag_id.'_'.$count.'_'.$exclude_key;
    $data = get_transient($transient_key);
    if ($data === false) {
        $args = array(
            'status' => 'publish',
        ...
        foreach ($products as $p) {
            $data[] = array(
                'id' => $p->get_id(),
                'name' => $p->get_name(),
                'image' => get_the_post_thumbnail($p->get_id(), 'woocommerce_thumbnail', array('loading'=>'lazy')),
...
wcc_record_tag_visit($tag_id);

$term_meta = get_term_meta($tag_id);
$tag_info = isset($term_meta['wcc_info'][0]) ? maybe_unserialize($term_meta['wcc_info'][0]) : array();
...
$rising_products = wcc_get_tag_rising_products_cached($tag_id, 4);
...
$tag_image_id = isset($term_meta['wcc_image'][0]) ? $term_meta['wcc_image'][0] : '';

$all_popular_products = wcc_get_mostpopular_products_for_tag_pages_cached($tag_id);

$term = get_term($tag_id);
$tag_name = $term->name;
$tag_description = $term->description;

$trending_tags = wcc_get_trending_tags_cached($tag_id, $selected_tags);

$total_popular_products = count($all_popular_products);
$initial_display_count = 12;
$product_count = get_product_count_in_tag_cached($tag_id);

/**
 * Preload der ersten 3 Produktbilder für die LCP-Optimierung.
 */
function wcc_optimize_first_tag_product_images($all_popular_products) {
...
wcc_optimize_first_tag_product_images($all_popular_products);
...
function render_optimized_product_image($product, $index) {
    ...
    // Erste 3 Bilder eager und sync, Rest lazy und async
...
/**
 * Optimiertes Laden des Tag-Bildes.
 */
function render_optimized_tag_image($tag_name, $tag_image_id) {
    if ($tag_image_id) {
        $tag_image_url = wp_get_attachment_image_url($tag_image_id, 'medium'); // Mittelgroß statt full
    } else {
        $tag_image_url = '';
    }
    $default_image = 'https://geschenkly.de/wp-content/uploads/2024/07/DALL·E-2024-07-26-16.28.35-Create-an-illustration-for-the-Gifts-for-Men-category-designed-to-fit-into-a-placeholder-space-with-a-friendly-and-inviting-vibe.-The-scene-should-.webp';
    ...
    printf(
        '<img width="180"
             ...
             loading="eager"
             decoding="sync">',
        esc_url($tag_image_url ?: $default_image),
        esc_attr($tag_name)
    );
}

/**
 * Progressive Image Loading (falls benötigt)
 */
...
$random_products = get_random_products_from_tag_cached($tag_id);
?>
<div class="tag-card-wrapper" id="tag-card" data-tag-id="<?php echo esc_attr($tag_id); ?>">
    <div class="tag-card">
        <div class="tag-content">
            <div class="tag-info-section">
                <div class="category-info-header">
                    <div class="category-image-wrapper">
                        <?php render_optimized_tag_image($tag_name, $tag_image_id); ?>
                    </div>
            ...
            <div>
              ...
              <?php if ($total_popular_products > $initial_display_count): ?>
              ...
              <div class="category-info-content">
                <div class="info-item">
                    ...
                    <div><?php echo wp_kses_post($tag_info['contents'] ?? ''); ?></div>
                </div>
                <div class="info-item">
                    <h3><i class="fas fa-users"></i> Für wen?</h3>
                    <div><?php echo wp_kses_post($tag_info['target_audience'] ?? ''); ?></div>
                </div>
                <div class="info-item">
                    <h3><i class="fas fa-birthday-cake"></i> Altersgruppe</h3>
                    <div><?php echo wp_kses_post($tag_info['age_group'] ?? ''); ?></div>
                </div>
                <div class="info-item">
                    <h3><i class="fas fa-star"></i> Was ist besonders?</h3>
                    <div><?php echo wp_kses_post($tag_info['special_features'] ?? ''); ?></div>
                </div>
              </div>
              <div class="category-info-footer">
                  <div class="trend-link">
                      ...
                      <?php if (!empty($trending_tags)): ?>
                          <?php foreach ($trending_tags as $related_tag): ?>
                              <?php
                              $related_tag_id = $related_tag['_id'];
                              $related_tag_name = $related_tag['tag_name'];
                              $related_tag_url = get_term_link((int) $related_tag_id, 'product_tag');
                              if (is_wp_error($related_tag_url)) {
                                  continue;
                              }
                              ?>
                              <a href="<?php echo esc_url($related_tag_url); ?>" class="trend-button"><?php echo esc_html($related_tag_name); ?></a>
                          <?php endforeach; ?>
                      <?php else: ?>
                          <span class="no-trend">Keine Trend-Tags verfügbar.</span>
                      <?php endif; ?>
                  </div>
                  <div class="pinterest-link">
                      ...
                      <?php
                      $pinterest_url = !empty($tag_info['pinterest_board_url'])
                          ? $tag_info['pinterest_board_url']
...
jQuery(document).ready(function($) {
    var tagId = <?php echo json_encode($tag_id); ?>;
    ...
    function generateProductHtml(product, tagId) {
        return `
            <div class="product-item" data-product-id="${product.id}">
                <div class="product-image-container">
                    ...
                    <div class="product-reaction-container">
                        <button class="reaction-button like-button" data-product-id="${product.id}" data-tag-id="${tagId}" aria-label="Dieses Produkt liken">
                            <i class="far fa-heart reaction-icon" aria-hidden="true"></i>
                        </button>
                        <button class="reaction-button dislike-button" data-product-id="${product.id}" data-tag-id="${tagId}" aria-label="Dieses Produkt nicht mögen">
                            <i class="far fa-thumbs-down reaction-icon" aria-hidden="true"></i>
                        </button>
                    </div>
                    <div class="awp-wishlist-button-container">
    ...
    function loadNewProducts() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wcc_get_random_products_from_tag',
                tag_id: tagId,
                count: 1,
            ...
            success: function(response) {
                if (response.success) {
                    var newProductHtml = generateProductHtml(response.data[0], tagId);
...
<h2>Alle Geschenkideen  - <?php echo $product_count; ?> <?php echo esc_html($tag_name);?></h2>
            ...
            <div class="product-item">
                <div class="product-image-container">
                    <a href="<?php echo get_permalink($wc_product->get_id()); ?>">
                        <?php echo get_the_post_thumbnail($wc_product->get_id(), 'woocommerce_thumbnail', $image_attributes); ?>
                    </a>
                    <div class="awp-wishlist-button-container">
                        <button class="awp-add-to-wishlist"
...
<?php
$index = 0;
foreach ($rising_products as $product):
$wc_product = wc_get_product($product['id']);
$button_text = $wc_product ? $wc_product->add_to_cart_text() : __('Zum Shop ➜', 'woocommerce');
$image_attributes = array(
    'decoding' => ($index < 3) ? 'sync' : 'async',
...
<div class="product-image-container">
    <a href="<?php echo esc_url(get_permalink($product['id'])); ?>">
        <?php echo get_the_post_thumbnail($product['id'], 'woocommerce_thumbnail', $image_attributes); ?>
    </a>
    <div class="awp-wishlist-button-container">
        <button class="awp-add-to-wishlist"
            data-product-id="<?php echo esc_attr($product['id']); ?>"
...
<div class="product-grid" id="sixMonthsPopularProducts">
    <?php
    $six_months_popular_products = wcc_get_tag_six_months_popular_products_cached($tag_id, 4);
    $index = 0;
...
<h2>Weitere Kategorien passend zu "<?php echo esc_html($tag_name); ?>"</h2>
<div class="subcategories-and-tags">
    ...
    if (!empty($selected_subcategories) || !empty($selected_tags)) {
        ...
        foreach ($selected_tags as $related_tag_id) {
            $related_tag = get_term($related_tag_id, 'product_tag');
            if ($related_tag && !is_wp_error($related_tag)) {
                $custom_text = isset($custom_tag_texts[$related_tag_id]) ? $custom_tag_texts[$related_tag_id] : $related_tag->description;
                $image_id = get_term_meta($related_tag_id, 'wcc_image', true);
                $image_url = $image_id ? wp_get_attachment_image_url($image_id, 'full') : '';
                echo '<a class="category-tag-card" href="' . esc_url(get_term_link($related_tag)) . '">';
                if ($image_url) {
                    echo '<div class="category-tag-image" style="background-image: url(\'' . esc_url($image_url) . '\');"></div>';
                } else {
                    echo '<div class="category-tag-image" style="background-image: url(\'' . esc_url('https://geschenkly.de/wp-content/uploads/2024/07/DALL·E-2024-07-26-16.28.35-Create-an-illustration-for-the-Gifts-for-Men-category-designed-to-fit-into-a-placeholder-space-with-a-friendly-and-inviting-vibe.-The-scene-should-.webp') . '\');"></div>';
                ...
                echo '<div class="category-tag-content">';
                echo '<h3>' . esc_html($related_tag->name) . '</h3>';
                echo '<p>' . esc_html($custom_text) . '</p>';
                echo '</div>';
                echo '</a>';
            }
...
<?php if (!empty($custom_description)): ?>
    <div class="custom-description-card">
        <h3>Wissenswertes zum Thema "<?php echo esc_html($tag_name); ?>"</h3>
...
<div class="filter-section category-insights">
	<h3>Unsere Partner-Shops für "<?php echo esc_html($tag_name); ?>"</h3>
	<div class="filter-buttons partner-grid">
		...
		if (is_array($selected_partner_shops) && !empty($selected_partner_shops) && !empty($partner_shops)) {
			foreach ($selected_partner_shops as $shop_index) {
				if (isset($partner_shops[$shop_index])) {
					...
					?>
					<a rel="sponsored" href="<?php echo esc_url($shop['url']); ?>"
					   class="filter-button partner-card"
					   target="_blank">
			...
			echo '<p>Keine Partner-Shops für diesen Tag ausgewählt.</p>';
		}
		?>
	</div>
...
<script>
var popularProductsOffset = <?php echo $initial_display_count; ?>;
var allPopularProducts = <?php echo json_encode($all_popular_products); ?>;
jQuery(document).ready(function($) {
  var tagId = <?php echo json_encode($tag_id); ?>;
  var tagName = <?php echo json_encode($tag_name); ?>;
  var risingProductsOffset = <?php echo count($rising_products); ?>;
  var sixMonthsPopularProducts = <?php echo json_encode(wcc_get_tag_six_months_popular_products_cached($tag_id, 4)); ?>;
  var sixMonthsPopularProductsOffset = <?php echo count(wcc_get_tag_six_months_popular_products_cached($tag_id, 4)); ?>;
  ...
  $('#tag-title').text(tagName || 'Alle Tags');
});
</script>
