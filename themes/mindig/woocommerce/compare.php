        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                <td class="<?php echo esc_attr( $product_class ); ?>">
                    <a href="<?php echo esc_url( add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->get_id() ) ) ) ?>" data-product_id="<?php echo $product->get_id(); ?>"><?php _e( 'Remove', 'yit' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
