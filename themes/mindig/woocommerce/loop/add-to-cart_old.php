    ...
    if ( yit_get_option( 'shop-add-to-cart-button' ) == 'yes' && yit_get_option( 'shop-enable' ) == 'yes' ) {
        ...
        <?php
            ...
            switch ( $handler ) {
                ...
                $link['label'] = apply_filters( 'external_add_to_cart_text', __( 'ZUM GESCHENK', 'yit' ) );
                    break;
                default :
            ...
            echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                sprintf( '<a rel="nofollow" target="_blank" onclick="ga(\'send\', \'event\', { eventCategory: \'Teaser\', eventAction: \'Klick\'});" href="/redirect.php?product=%s" class="btn btn-flat %s">%s &#10132;</a>',
                    esc_attr( $product->post->post_name ),
                    esc_attr( $link['class'] ),
                    $link['label'] ),
                    $product );
        endif;
