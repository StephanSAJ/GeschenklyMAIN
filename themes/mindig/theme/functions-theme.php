...
function yit_setup_theme() {
    ...
    // Default Sidebar
    register_sidebar( yit_sidebar_args( "Default Sidebar", __( "The default widgets area ready to use.", 'yit' ), 'widget', 'p' ) );

    // Mobile Sidebar
    register_sidebar( yit_sidebar_args( "Mobile Sidebar", __( "The widgets area used in the side menu of mobile, in bottom part. The widgets compatible with this are widget are: YIT Woocommerce Login, Text and Custom Menu.", 'yit' ), 'widget', 'p' ) );
...
if ( version_compare( $GLOBALS['wp_version'], '5.6-alpha', '<' ) && ! class_exists( 'Yit_Migrate_Helper' ) ) {
	include_once 'class.migrate-helper.php';
	add_action( 'after_setup_theme', array( 'Yit_Migrate_Helper', 'init_actions' ) );
}