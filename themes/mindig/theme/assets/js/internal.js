...
setTimeout(fixBigMenuPosition, 0);
    }
...
setTimeout(fixBigMenuPosition, 0);
    ...
    $.fn.yit_parallax = function () {
        this.each(function () {
            ...
            container.find('.vertical_center').css({
                top: vc + 'px',
                marginBottom: 'auto'
            });
    ...
    var SidebarMenuEffects = (function () {
        ...
        function init() {
            ...
            buttons.forEach(function (el, i) {
                ...
                addListenerMulti(el, eventtype, function (ev) {
                    console.log(ev);
                    ev.stopPropagation();
                    ev.preventDefault();
    ...
    /* create filter trigger and show button back to top*/

    if ($('#header-sidebar .widget_text').length > 0 || $('body').hasClass('tax-product_tag')) { // Arthur's fix for displaying filter on mobile
      $('#logo').after($('<div id="mobile-filter-trigger" class="mobile-filter-trigger" data-effect="st-effect-4" />'));
        $('#back-top').css('z-index', '1000');
    }
    ...
    var SidebarFilterEffects = (function () {
        ...
        function initMarkup() {
            ...
            $('#st-container').prepend('<nav class="st-filter st-effect-4" id="menu-2" />');
            $('.st-filter').after($('<div class="st-filter-bg" />'));


            // change .nav class to .mobile-nav
            if (sidebar) sidebar.find('.nav').removeClass('nav').addClass('mobile-nav');

            $('nav.st-filter').append(sidebar);
        }
        ...
        function init() {
            ...
            $('#mobile-filter-trigger').on('click', function () {
                ...
                $('.st-filter-bg').css('display', 'block');
                $('.st-filter-bg').animate({
                    opacity: 0.75
                })
                $('.st-filter').animate({
                    right: 0
            ...
            $('.st-filter-bg').on('click', function () {
                $('.st-filter-bg').css('display', 'none');
                $('.st-filter-bg').animate({
                    opacity: 0
                })
                $('.st-filter').animate({
    ...
    var ElementsOnSinglePageOrder = function () {
        if (window.innerWidth < 769) {
            $('.woocommerce-Tabs-panel.woocommerce-Tabs-panel--description.panel.entry-content.wc-tab').find('h3+ul').addClass('item_page_description_list');
            var marked_list = $('.item_page_description_list');
            $('.woocommerce-Tabs-panel.woocommerce-Tabs-panel--description.panel.entry-content.wc-tab').find('h3:eq(1)').css('display', 'none');

            $('[itemprop="offers"]').after(marked_list);
            $('[itemprop="description"]').before($('p.cart'));
            $('[itemprop="description"]').after($('.product .images'));
        }
    }
    var createButton = function () {
        if (window.innerWidth < 769) {
            var fixed_button = $('p.cart').clone().addClass('fixed_order_button').appendTo('body');
        }
    }
    createButton();
    var HideButton = function () {
        var fixed_button = $('.fixed_order_button');
    ...
    var ManageButton = function () {
        var fixed_button = $('.fixed_order_button');
        var manage_marker = true;
        if (fixed_button.length) {
            if (window.pageYOffset > 100 && manage_marker) {
                manage_marker = false;
                fixed_button.fadeIn();
            } else {
                manage_marker = true;
    ...
    setTimeout(ElementsOnSinglePageOrder, 0);
    window.addEventListener('resize', ElementsOnSinglePageOrder);

    var MenuForTablets = function () {
        if (window.innerWidth < 769) {
            $('.sidebar-left[role="secondary"]').hide();
            if (window.innerWidth == 768) {
                $('.content[role="main"]').css('margin', 'auto 80px');
                $('.content[role="main"]').css('width', '75%');
            }
            $('.content.clearfix').removeClass('col-sm-9').removeClass('col-sm-push-3');
            $('#mobile-menu-trigger >a').removeClass('visible-xs');
        }
    ...
    /*   Item page - add wishlist button to header  */


/*    function changeWishlistPlace() {
        if ($('.product-actions.count-1') && !$('#header-sidebar .widget_text').length > 0) {
            $('#header-container').append('<a class="header_wishlist" />');
            var href = $('.product-actions.count-1 .show >a').attr('href');
            $('a.header_wishlist').attr('rel', 'nofollow');
            $('a.header_wishlist').attr('href', href);
            $('.product-actions.count-1').remove();
        }
    }
    ...
    /* Item page - changing menu button */

    /*var changingMobileMenuButton = function () {
        $('#mobile-menu-trigger').removeClass('mobile-menu-trigger').addClass('item_page__arrow_go_back').find('a').removeAttr('class')
    }
    changingMobileMenuButton();*/
