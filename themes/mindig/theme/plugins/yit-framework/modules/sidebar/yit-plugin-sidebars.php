...
class YIT_Plugin_Sidebar {
    ...
    public function add_setting_page() {
        ...
        $fw_slug = 'yith_plugin_panel';
        if ( !isset( $admin_page_hooks[ 'yith_plugin_panel' ] ) && isset( $admin_page_hooks[ 'yit_plugin_panel' ] ) ) {
            $fw_slug = 'yit_plugin_panel';
        }

        if ( !isset( $admin_page_hooks[ $fw_slug ] ) ) {
            $position = apply_filters( 'yit_plugins_menu_item_position', '62.32' );
            add_menu_page( $fw_slug, __( 'YITH', 'yit' ), 'nosuchcapability', $fw_slug, NULL, $this->plugin_url . '/assets/images/yithemes-icon.png', $position );
        }
        add_submenu_page( $fw_slug, __( 'YITH Sidebars', 'yit' ), __( 'Sidebars', 'yit' ), 'manage_options', 'yit_sidebar_panel', array( $this, 'yit_sidebar_panel' ) );
    }
        ...
        if( !empty($this->custom_sidebars)){
            foreach($this->custom_sidebars as $sidebar){
                register_sidebar( array(
                    ...
                    'description' => '',
                    'class' => apply_filters('yit_sidebar_class', 'wi'),
