        ...
        if ( $type == 'text' ) {
            ...
            foreach ( $socials as $i => $social ) {
                ...
                if ( $social == 'facebook' ) {
                    ...
                    $attrs = " rel=\"nofollow\" onclick=\"javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;\"";
                }
                else {
                    if ( $social == 'twitter' ) {
                        $url = apply_filters( 'yiw_share_twitter', 'https://twitter.com/share?url=' . $permalink . '&amp;text=' . $title . '' );
                        $attrs = " rel=\"nofollow\" onclick=\"javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=417,width=600');return false;\"";
                    }
                ...
                <a href='<?php echo $url; ?>' rel="nofollow" class="socials-text <?php echo $social ?>" target="_blank" <?php echo $attrs ?>>
                    <?php _e( 'Share on ', 'yit' ); ?>
                    <?php echo $social; ?>
                </a>
...
if( ! function_exists('yit_page_title') ){
    ...
    function yit_page_title(){
        ...
        if ( is_archive() ) {
           ...
           elseif ( is_category() ) {
                return sprintf( __( '%s', 'yit' ), single_cat_title( '', false ) );
            }
...
// Check standard first and last name fields
        if ( $current_user->user_firstname && $current_user->user_lastname ) {
            $user_name = $current_user->user_firstname . ' ' . $current_user->user_lastname;
        }
        // If not available, use the user's nice name
        else {
