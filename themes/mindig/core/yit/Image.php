...
class YIT_Image extends YIT_Object {
    ...
    public function __construct() {
        ...
        if ( function_exists('w3_instance') ) {
            ...
            if ( is_object( $config ) && $config->get_boolean('cdn.enabled') ) {
                $this->is_onfly_active = false;
                return;
            }
        }

        //deactivate if wp_rocket cdn is enabled
    ...
    public function image( $args = array(), $echo = true ) {
        ...
        if ( ( ! empty( $width ) || ! empty( $height ) ) && ! $is_external ) {
            ...
            if ( $this->is_onfly_active ) {
                ...
                // STEPHAN Änderung Überprüfen Sie, ob die Schlüssel im Array vorhanden sind
                  if (isset($vt_image['url'])) {
                      $image_url = $vt_image['url'];
                  }

                  if (isset($vt_image['width'])) {
                      $width = $vt_image['width'];
                  }

                  if (isset($vt_image['height'])) {
    ...
    protected function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false, $is_retina = false ) {
        ...
        if ( $attach_id ) {
            ...
            if ( $img_url ) {
                ...
                // Änderung Stephan Überprüfen, ob getimagesize gültige Daten zurückgegeben hat
                  if ( $orig_size !== false ) {
                      $image_src[0] = $img_url;
                      $image_src[1] = $orig_size[0];
                      $image_src[2] = $orig_size[1];
                  } else {
                      // Fehlerbehandlung oder Standardwerte festlegen
                      $image_src[0] = $img_url;
                      $image_src[1] = 0; // Standardbreite
                      $image_src[2] = 0; // Standardhöhe
                  }
                  // Änderung Ende
            }
        }
